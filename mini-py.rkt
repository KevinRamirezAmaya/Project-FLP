...
;;                  ;; <for-exp (id iterable body)> 
;;               | <list>
;;                  ;; <list-exp (elems)>
;;               | <tuple>
;;                  ;; <tuple-exp (elems)> 
;;               | <record>
;;                  ;; <record-exp (fields vals)>
;;               | <bool-expr>
;;                  ;; <bool-exp (boolop args)>
;;               | <circuit>
;;                  ;; <circuit-exp (gates)> 
;; <bool-expr> ::= <pred> ( <expression> , <expression> )
;;                 ;; <pred-exp (op arg1 arg2)> 
;;              | <bool-bin-op> ( <expression> , <expression> )
;;                 ;; <bool-bin-exp (op arg1 arg2)>
;;              | <bool-un-op> ( <expression> )
;;                 ;; <bool-un-exp (op arg)>
;; <primitive> ::= + | - | * | / | % | add1 | sub1
;;               | longitud | concatenar
;;               | vacio? | vacio | crear-lista | lista? | cabeza | cola | append | ref-list | set-list
...
;;             ;; <list-exp (elems)>
;;
;; <tuple> ::= tupla [ <expression> (; <expression>)* ]
;;             ;; <tuple-exp (elems)>
;;
...
;;              ;; <gatelist (gate rest)>
;; 
;; <gate> ::= <identifier> <type> <inputlist>
;;            ;; <gate (id tipo inputs)>
;; 
;; <type> ::= and | or | not | xor
;; 
...
;;                 ;; <empty-inputlist>
;;              | <bool> <inputlist>
;;                 ;; <bool-inputlist (bool rest)>
;;              | <identifier> <inputlist>
;;                 ;; <id-inputlist (id rest)>
;; 
;; <bool> ::= True | False
;; 
;; <string> ::= '"' .* '"'
...
(define grammar-mini-py
  '((program ((arbno class-decl) expression) a-program)
    ...
    (primitive ("longitud") str-length-prim)
    (primitive ("concatenar") str-concat-prim)
...
;;                                  "Attempt to apply non-procedure ~s" proc))
      ...
      (for-exp (var start-exp end-exp body)
         (let ((start-val (eval-expression start-exp env))
           ...
           (let loop ((i start-val) (iteration 0) (result 0))
             (if (<= i end-val)
                 (let* ((new-env (extended-env-record 
                                 (list var '__iter '__count '__total) 
                                 (vector i iteration (+ iteration 1) (+ (- end-val start-val) 1)) 
                                 env))
                        (body-result (eval-body body new-env)))
                   (loop (+ i 1) (+ iteration 1) body-result))
                 (+ (- end-val start-val) 1)))))
    ...
    (cases primitive prim
      ...
      (str-length-prim ()
                       (let ((str (car args)))
                         (if (string? str)
                             (string-length str)
                             (eopl:error 'str-length-prim "Argumento no es una cadena: ~s" str))))
...
(define eval-body
  (lambda (exps env)
    (if (null? exps)
        0 ; valor por defecto si el cuerpo está vacío
        (let loop ((exps exps) (result 0))
          (if (null? exps)
              result
              (let ((current-result (eval-expression (car exps) env)))
                (loop (cdr exps) current-result)))))))
